...
- Keep the mobile Monitor view, Market Signals, and Paper Bot state in separate tabs; keep PWA manifest/meta injection in `app.py` and theme in `.streamlit/config.toml`.
...
2. Inspect `app.py`, `src/config.py`, `src/binance_client.py`, `src/indicators.py`, `src/signals.py`, `src/analysis_engine.py`, `src/paper_worker.py`, `src/storage.py`, `src/paper_trading.py`, `src/risk.py`, `src/strategy.py`, `src/ml.py`, and `tests/`.
